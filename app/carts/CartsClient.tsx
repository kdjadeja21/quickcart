        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {carts.map(cart => {
              ...
              return (
                <Card key={cart.id} className="group hover:shadow-xl transition-all duration-300 bg-gradient-to-br from-white via-indigo-50/30 to-purple-50/30 dark:from-slate-900 dark:via-slate-800/50 dark:to-slate-700/50 border-0 shadow-md hover:scale-[1.02]">
                  <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-4">
                    ...
                    <div className="ml-4 self-start flex flex-col items-end gap-3">
                      <span
                        className={
                          `rounded-full border px-3 py-1 text-xs font-semibold transition-all ${cart.status === 'active'
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          aria-label={cart.status === 'active' ? 'Open cart' : 'View cart'}
                          title={cart.status === 'active' ? 'Open cart' : 'View cart'}
                          onClick={() => {
                  ...
                  <CardContent className="pt-0">
                    <div className="space-y-2">
                      {cart.items.slice(0, 3).map((it, index) => (
                        <div key={it.id} className="flex items-center justify-between p-2 rounded-lg bg-gray-50/50 dark:bg-slate-800/30 hover:bg-gray-100/70 dark:hover:bg-slate-700/50 transition-colors">
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            <span className="text-xs font-medium text-muted-foreground bg-white dark:bg-slate-700 px-1.5 py-0.5 rounded-full">
                              {index + 1}
                            </span>
                            <span className="truncate text-sm font-medium text-gray-700 dark:text-gray-300">{it.name}</span>
                          </div>
                          <span className="text-sm font-semibold text-gray-900 dark:text-gray-100 ml-2">
                            {it.quantity} × {formatCurrency(it.price, currentCurrency)}
                          </span>
                        </div>
                      ))}
                      {cart.items.length > 3 && (
          ...
          <DialogContent className="max-w-md">
            <DialogHeader className="text-center">
              ...
              <DialogDescription className="text-gray-600 dark:text-gray-400 mt-2">
                This action cannot be undone. The cart <span className="font-semibold text-gray-900 dark:text-gray-100">"{cartToDelete?.name || 'Untitled Cart'}"</span> will be permanently deleted along with all its items.
              </DialogDescription>
            </DialogHeader>
